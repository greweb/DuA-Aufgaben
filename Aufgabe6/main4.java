...
public class main4 {
	...
	public static void main(String[] args) 
	{
		...
		int gueltiglength = 0;
		int l= 0 ;
		while (l<gueltig.length)
		{
			if (gueltig[l]!= null)
				gueltiglength++;
			l++;
		}
		System.out.println(gueltiglength);
		Comparable [] Sortierwerte = new Comparable[gueltiglength];
		int i = 0;
		l= 0 ;
		while (l<gueltig.length)
		{
			if (gueltig[l]!= null) {
				Sortierwerte[l] = (Comparable) gueltig[i];
				i++;
				}
			l++;
		}
		...
		Sortierwerte = sort.mergesort(Sortierwerte);
		int m = 0;
		while(m<gueltiglength) {
			System.out.println((m+1)+".: Wert: "+((Zahlungsmittel)Sortierwerte[m]).getWert()+" Waehrung: "+((Zahlungsmittel)Sortierwerte[m]).getWaehrung());
			m++;
		}
